...
function (error) {
  console.log("Failed to send the messsage. Please try again.");
}
);
});
document.getElementById('inputText').addEventListener('input', function(){
  let value = this.value;
  this.value = value.chartAt(0).toUpperCase() + value.slice(1).toLowerCase();
});


const contactDetails = document.getElementById('contact-deatils');
...
requestAnimationFrame(() => {
  ...
  newX = Math.max(0, Math.minWindow.innerWidth - contactDetails.offsetWidth, newX);
  newY = Math.max(0, Math.minWindow.innerHeight - contactDetails.offsetHeight, newY)
